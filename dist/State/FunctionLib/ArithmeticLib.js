...
class ADD extends FunctionBlock {
    constructor() {
        super(ArithmeticLibDefinitions.ADD);
        this.run = (inputs) => inputs.reduce((output, input) => output += input, 0);
    }
}
class SUB extends FunctionBlock {
    constructor() {
        super(ArithmeticLibDefinitions.SUB);
        this.run = ([a, b]) => a - b;
    }
}
class MUL extends FunctionBlock {
    constructor() {
        super(ArithmeticLibDefinitions.MUL);
        this.run = (inputs) => inputs.reduce((output, input) => output *= input, 1);
    }
}
class DIV extends FunctionBlock {
    constructor() {
        super(ArithmeticLibDefinitions.DIV);
        this.run = ([a, b]) => (b == 0) ? [0, 1] : [a / b, 0];
    }
}
